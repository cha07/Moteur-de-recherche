<?php
	...
	if((strtolower(filter_input(INPUT_SERVER, 'HTTP_X_REQUESTED_WITH')) === 'xmlhttprequest') && ($_SERVER['REQUEST_METHOD'] == 'GET')){
		...
		/* connexion SQL  (avec PDO car Mysql_connect sera déprécié dès php 7) */
		$host='localhost';
		$port='3306';
		$database='france';
		$user='root';
		...
		$query=$connexion->query('SELECT ts.id, ts.name, ts.department, ts.lien_id FROM city ts WHERE (ts.name LIKE \'%'.$q.'%\' OR ts.name LIKE \'%'.$qTiret.'%\' OR ts.name LIKE \'%'.$qSpace.'%\' OR ts.department LIKE \'%'.$q.'%\') ORDER BY ts.name ASC');
		...
		while($q = $query->fetch()){
			...
			$department = utf8_encode($q->department);
			$array[] = array(
					'id' => $q->id,
